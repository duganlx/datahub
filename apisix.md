...
https://apisix.apache.org/zh/docs/apisix/pubsub/kafka/

Apache APISIX version 3.5

目前仅仅提供 `ListOffsets` 和 `Fetch` 两个API去拉取Kafka消息, 暂不支持消费者组特性, 也不能管理偏移量. 

[交互协议](https://github.com/apache/apisix/blob/master/apisix/include/apisix/model/pubsub.proto) 内容如下. 其中, 请求中的`sequence`用于关联响应. CmdKafkaListOffset 中的 `timestamp` 可以是 `-1, -2, unix timestamp`. 

将 proto文件变成成 ts, 可以使用 https://github.com/thesayyn/protoc-gen-ts
